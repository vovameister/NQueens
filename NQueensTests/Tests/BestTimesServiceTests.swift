...
final class BestTimesServiceTests: XCTestCase {
    ...
    func testAddResultSavesAndCleansUp() async {
        ...
        let savedRecords = mockStore.state.savedRecords
        let deletedForSizes = mockStore.state.deletedForSizes
    ...
    func testGetBestTimeReturnsFirstRecord() async {
        mockStore.state.setRecords([RecordDTO(id: UUID(), boardSize: 8, time: 12.3, date: Date())])
    ...
    func testIsNewRecordReturnsTrueWhenBetter() async {
        mockStore.state.setRecords([RecordDTO(id: UUID(), boardSize: 8, time: 20.0, date: Date())])
    ...
    func testIsNewRecordReturnsFalseWhenWorse() async {
        mockStore.state.setRecords([RecordDTO(id: UUID(), boardSize: 8, time: 10.0, date: Date())])
    ...
    func testGetResultsMapsRecordsToGameRecords() async {
        let id = UUID()
        let date = Date()
        mockStore.state.setRecords([RecordDTO(id: id, boardSize: 6, time: 25.5, date: date)])
    ...
    func testGetResultsHandlesNilIdAndDate() async {
        mockStore.state.setRecords([RecordDTO(id: nil, boardSize: 10, time: 5.0, date: nil)])
